...
class Settings extends Component {
	constructor(props) {
		...
		Object.keys(this.props.recommendations).forEach((key) => {
			if (parseFloat(this.props.recommendations[key]) !== 0) {
			  this.state = { ...this.state, [key]: parseFloat(this.props.recommendations[key]) }
		  } else {
	...
	handleSaveRecommendationButton = async () => {
		const response = await dataservice.setRecommendedValuesForUser(this.props.user.token, this.state)
  		  ...
  		  <Grid.Row>
	  	            ...
	  	            columns={[
	                    ...
	                    Cell: row => (
	                      <input 
	                        ...
	                        style={ 
	                        	this.state[row.original.koodi.toLowerCase()] > 0 ? tablestyles.inputRecommendationSet : tablestyles.inputRecommendationNotSet 
	                        }
	  		...
	  		<Grid.Row>
  		  	<Grid.Column width={16}>
  		  		<Button positive size="tiny" onClick={ this.handleSaveRecommendationButton }>Tallenna</Button>
  		  	</Grid.Column>
  		  </Grid.Row>
  		</Grid>
  	)
